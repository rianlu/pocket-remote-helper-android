...
public final class WsHttpServer {
    private static final String TAG = "PocketRemoteNet";
    ...
    private String readWsText(InputStream in, OutputStream out) throws Exception {
        int b0 = in.read();
        int b1 = in.read();
        if (b0 < 0 || b1 < 0) {
            return null;
    ...
    private void writeWsControl(OutputStream out, int opcode, byte[] data) throws Exception {
    ...
    private void serveHttp(String method, String path, Map<String, String> headers,
        ...
        if ("GET".equals(method) && "/apps/apk".equals(p)) {
            String pkg = query.get("pkg");
            if (pkg == null || pkg.length() == 0) {
                writeHttp(out, 400, "text/plain", "pkg".getBytes("UTF-8"));
                return;
            }
            File apk = commands.extractApk(pkg);
            if (apk == null || !apk.isFile()) {
                writeHttp(out, 403, "text/plain", "apk".getBytes("UTF-8"));
                return;
            }
            writeHttpHeader(out, 200, "application/vnd.android.package-archive", apk.length());
            store.copyTo(apk, out);
            out.flush();
            apk.delete();
            return;
        }
